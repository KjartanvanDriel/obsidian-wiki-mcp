...
This is a structured Obsidian vault. A human curates the content in Obsidian. Agents operate through slash commands, each with its own role.
...
- **IMPORTANT: Target your commits.** `wiki(action="commit")` without a `files` list runs `git add -A` and sweeps everything in the working tree — including the user's unrelated in-progress edits. This produces muddled commits and can accidentally publish half-finished work. Always pass `files=[...]` with the specific paths your change touched. Omit `files` only when the user explicitly says "commit everything" or when you've verified the tree contains nothing but your change. Example: `wiki(action="commit", message="Fix X", files=["knowledge/tools/jax.md"])`.
